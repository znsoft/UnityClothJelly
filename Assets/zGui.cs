...
public class TrackNode
{
    ...
    private void CalcPattern()
    {
        ...
        if (patterns == null) patterns = CalcPatLen();
...
public class zPlayer
{
    ...
    void OnAudioFilterRead(float[] data, int channels)
    {
        ...
        for (var i = 0; i < data.Length; i = i + channels)
        {
            ...
            List<zChannel> chans = seq[pos].channels;
...
public class zInstrument
{
    ...
    zSynthGen synth;
    public float length;
    public zInstrument(zInstrumentType type)
    {
    ...
    private void GenSampleByType(float phase, float freq)
    {
        switch (type)
        {
